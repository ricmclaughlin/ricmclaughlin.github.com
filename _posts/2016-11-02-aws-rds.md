...
Each reservation is associated to a specific DB Engine, instance class, deployment type, license model and region (NOT AZ). If any of these attributes are changed then pricing switches to on-demand pricing. Switch your instance BACK to the specs of the reservation OR create a new instance with the same specs, reservation pricing will apply. And yes, this also applies to read replicas.
