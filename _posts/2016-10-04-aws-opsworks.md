...
Layers define which recipes are installed, networking (ELB, Public IP address), EBS Volumes, security groups, EC2 instance profile and tags. There is also a shutdown timeout associated to layers
...
[Databags](https://docs.chef.io/data_bags.html) are global JSON objects which can be defined on the stack, layer, app, or instance lebels.  Because there is no Chef server they are defined at the command line OR using the CUSTOM_JSON field.
...
Load based scaling is based on setting Up and Down layer averages for CPU, memory or load and ONLY works for load based instances. There is a batch size, threshold exceeded timeframe &amp; after scaling ignore timeframe (cooldown) for both Up and down. 
...
You can associate EIP, volumes, and RDS instances to a stack. You can only associate a volume with a stopped instance.
